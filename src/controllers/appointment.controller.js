...
exports.getSlotBookings = (req, resp) => {
    ...
    Appointment.findAll({where:{opdid:opdId, date:date}, attributes:['slot'], raw:true})
    .then(data => {
...
exports.getAllAppointments = (req, resp) => {
    const {opdId, date} = req.query;
    Appointment.findAll({where:{opdid:opdId, date:date}, include:[{model:Patient, attributes:['id'], include:[{model:User}]}], raw:true})
    .then(data => {
        resp.status(201).send(data);
    })
    .catch(err => {
...
exports.getAppointmentsByFilter = (req, resp) => {
    Appointment.findAll({where:req.query, include:[{model:Patient, attributes:['id'], include:[{model:User, attributes:['id', 'firstname', 'lastname', 'email', 'phone', 'dob']}]}, {model: Opd, attributes:['id'], include:[{model:Doctor, attributes:['id'], include:[{model:User, attributes:['id', 'firstname', 'lastname', 'email', 'phone']}]}]}], raw:true})
    .then(data => {
        resp.status(201).send(data);
    })
    .catch(err => {
...
exports.deleteAppointment = (req, resp) => {
    ...
    .then(result => {
        if(result === 1){
            ...
            resp.status(500).send({
                message:
                    "Some error occurred while deleting Appointment."
                });
        }
    })
    .catch(err=> {
